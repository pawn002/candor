...
| ~~BL-5~~ | ~~`candor-slider` `Percentage` & `GradientTrack` stories announce the raw number ("70", "0.65") with no unit~~ — **Fixed**: `valueTextFn` is a function and cannot pass through a Storybook `template:` string, so the stories never set it. Added a shared `setElementProps()` `play` helper (`src/web-components/story-utils.ts`) and applied it to set `valueTextFn` after render — `aria-valuetext` now reads "70%" / "L=0.65". Component was already correct; story-only fix. (`valueTextFn` is the only function-valued property in the library, so this closes the theme.) | Slider stories | Low–Med | Fixed |
| ~~BL-6~~ | ~~`Design Tokens / Colors → Token Reference` — 16 section labels were `<p>` (not headings) and the 16 tables had no accessible name~~ — **Fixed**: section labels are now `<h2>` (heading navigation works) and each `<table>` is linked to its heading via `aria-labelledby`. Story-only. | Colors Token Reference story | Low | Fixed |
| ~~BL-7~~ | ~~`candor-tone-picker` out-of-gamut cells rendered as empty `<td role="gridcell">` — silent in browse mode~~ — **Fixed**: out-of-gamut cells now carry `aria-label="Out of gamut"`, so an SR user hears "L …, C …, Out of gamut" instead of a confusing blank. Matches the DataGrid pattern and the PR-checklist "label empty/disabled cells" rule. Component change. | TonePicker | Low | Fixed |
...
**Phase 5 update:** The `--quiet` variant heading was changed from regular to bold weight to meet Tier 2 contrast requirements (OKCA 4.5 bold at 14px — `--color-text-subtle` at 5.0 passes bold but not the 9.5 regular threshold). No ARIA or structural change. See `docs/CONTRAST-TIERS.md`.
