...
- **`@candor-design/web-components` now ships a Custom Elements Manifest at `custom-elements.json`, declared via the `customElements` field (#267).** VS Code and JetBrains read it for tag and attribute completion; it is trivially readable by an agent; and it is the machine-readable form of the whole element surface — 40 elements, all with descriptions, 27 events, 14 CSS parts and 24 custom properties.

  It is worth shipping **only because the TSDoc pass landed first.** Generated from undocumented source it would have been an accurate list of names, which is roughly what the `.d.ts` already provide — the issue's own caveat, and the reason this was sequenced second.

  `cssParts` and `cssProperties` are **derived from the source** rather than from hand-written `@csspart`/`@cssprop` tags. A `part="input"` in the template already *is* the declaration, and CLAUDE.md makes part names public API under semver, so a tag that drifts from the template is a false promise rather than a stale comment. Attribution is per class node, not per file: `candor-toast.ts` defines two elements and its `--candor-toast-*` properties belong to `CandorToast`, which a file-level scan would credit to both. The derived figures independently reproduce #246's hand-recorded coverage — 5 elements expose a part, 9 expose a custom property, 4 expose both, 27 expose neither.

  Private and protected members are stripped: 214 of the 403 the analyzer collects are internals like `_onKeydown`, and a published manifest that lists them invites the inference this whole issue is about.

  **Size, measured rather than assumed.** The issue estimated this as "negligible against a 170 kB bundle"; it is not quite that. The manifest is 181 kB raw / 27 kB gzipped and adds ~25 kB to the tarball (118.0 → 143.3 kB), a 21% increase. Still clearly worth it, and worth being accurate about: it is a dev-time artifact that editors and tooling read, so it never reaches a browser and does not affect the shipped bundle at all.

- **A class-level TSDoc block on every one of the 40 registered custom elements, with `@fires` declaring every event (#267).** The build already emits these into `dist/*.d.ts`, so there is no new artifact and nothing to keep in sync — the comment travels with the declaration it describes. This is the channel that matters for the failure the issue is about: types are what a consumer reads *at the moment of the decision*, where a link to the catalog is a step that does not get taken.

  **Coverage is uniform rather than targeted, and that was the substantive design call.** Documenting only the highest-risk components would have been cheaper and is worse in two ways. It manufactures a false signal — once most siblings carry a block, a missing one starts to read as "nothing surprising here," which is the #213 half-filled-column problem — and it is permanently ungateable, since "every element declares its events" only means something if every element is supposed to. The apparent counter-precedent, #246's rule that the `::part` surface grows by demand and is never populated blind, does not transfer: a part name is public API under semver, and a comment commits to nothing.
...
- **`npm run audit:docs` gates the README's claims about the component surface against what `src/` actually registers (#267).** Checks the tag table in both directions and every prose count. Runs in CI in the `audit` job — an existing *required* status check, so it blocks on merge rather than only reporting.
  ...
  **The drift it was filed for turned out not to be drift.** Three counts were in circulation at 5.0.1 — "37 custom elements" in README prose, 40 tags in that same README's table, 41 in issue #257's title, the last having already propagated into a consumer's own notes. But **37 and 40 are both correct**: there are 37 component source files and 40 registered custom elements, because `candor-tabs`, `candor-toast` and `candor-toolbar` each register a companion element. The obvious fix — 37 → 40 everywhere — would have broken the hook-coverage measurements in `CLAUDE.md` and `Introduction.mdx`, whose denominator is components and whose parts sum to exactly 37.
  ...
  It also gates the TSDoc surface: a class-level block on every registered element, every `@fires` tag against the `new CustomEvent` calls in the same file in both directions, and — since absence has to be asserted rather than inferred — the exact words `Emits no custom events.` on any element that dispatches none. Prose is deliberately **not** checked for truth; only the two mechanical halves are gated, so the check does not imply it verified something it cannot.

  And it gates the README's library-wide event list, **which had already shipped wrong in this same release.** The list named six events when the source dispatches nine; the three it missed (`cell-activate`, `color-select`, `send`) are all written as `new CustomEvent<Detail>('name')`, and the scan used to derive it required `(` immediately after `CustomEvent`. A hand-derived list that nothing checked, wrong within a day — which is the argument for gating it rather than re-deriving it more carefully.

  It also gates the shipped manifest, in two complementary halves that neither subsumes: `audit:docs` asserts the committed manifest describes exactly the registered elements and that none is undescribed, while CI regenerates it and fails on a diff. The first catches a manifest generated from a glob that silently stopped matching — which produces a smaller file that diffs clean against its own regeneration; the second catches a component changed without regenerating.
...
- **Dark-mode visual coverage: `.storybook/modes.ts`, and `chromatic.modes` on ten story files (#286, #287).** Candor renders every component in light and dark from the same tokens, but the visual gate only ever saw light — of 248 snapshots exactly one covered dark, and it was `color-showcase`'s swatch grid rather than a component. `audit:contrast` measures both modes, so the *figures* were checked; nothing had ever looked at a dark-mode component, so a dark-only regression breaking no contrast floor would have shipped in silence. The suite goes 248 → 292, and the ten files cover all 37 theme-varying tokens any story actually paints.

  **Per-story rather than global in `preview.ts`, because a mode is a separately billed snapshot.** Applying it project-wide would take the suite to 496 and cost more per quarter than the TurboSnap work above saved. Which stories earn dark is a judgement per story, and deliberately **not** gated — nothing checks that a component seen only in light is one that ought to be.

  **Coverage is counted by token with the variant gates honoured**, which is the load-bearing part: a story is credited only with tokens whose CSS gate it satisfies, so base rules always but a `.x--variant` rule only when the story renders that variant. Counting tokens merely *present* in a component's stylesheet gives 50 against the 37 genuinely painted, and named three stories to add where the variant-aware count says five. The cheap version of the analysis would have shipped a hole.

  **Two behaviours found by measurement rather than assumed, both now written down.** Adding modes changes a snapshot's *identity* — a story that produced one unnamed snapshot now produces two named `light` and `dark`, both new keys — so the light one re-baselines despite being pixel-identical; the #286 prediction of 27 baselines was wrong and the answer was 54. And *any* edit to a file in `.storybook/` disables TurboSnap for that build — including a comment-only one, since the trigger is the path rather than the content. Measured twice: a one-word comment fix in `modes.ts` captured all 292, and so did the comment added to `main.ts` by the `externals` gate below.

  **The ceiling is recorded because it is not a gap to close.** Five tokens are painted only on `:hover`/`:active` — `--color-action-destructive-hover`/`-active`, `--color-action-primary-hover`/`-active`, `--color-link-hover`. No resting-state snapshot reaches them in *either* theme, so "dark coverage is complete" means complete for what snapshots can reach. Conversely `--color-focus` looks interaction-only and is not: `candor-tone-picker` paints it as the border of the selected swatch, so it is reachable at rest.

- **Two Chromatic decisions recorded where they will be read, rather than only in the issues that produced them.**

  `.claude/CLAUDE.md` gains **branch names are single-use** (#284, #285). Chromatic picks its ancestor build by branch name, so reusing one after its PR squash-merges points the ancestor at the previous incarnation — whose commit is not reachable from `main` — and git's merge-base falls back to before that PR landed. Measured on the same one-file diff: **97 changed files and 248 captured** on a reused name against **1 changed file and 248 bypassed** on `main`. The blast radius is whatever the intervening squashed content touched, so reuse after a docs-only merge still bypasses correctly — which makes it *intermittent*, and worse than consistently broken.

  The `chromatic` job comment gains **why the `pull_request` trigger stays** (#288). Chromatic's docs say to trigger on `push` because TurboSnap otherwise examines ephemeral merge commits; the premise is true here and the consequence is not. `actions/checkout` does check out the merge ref, but `chromaui/action` overrides the SHA and branch before the CLI reads them, so the CLI reported the PR head on the real branch name and exactly the PR's own six files. #281 had flagged this as a prediction to check against a real build log before acting; this checks it and changes nothing but the comment. The comment also names the condition to re-check under — the doc line is written for workflows invoking the CLI directly, where no override exists.

- **`audit:docs` gates Chromatic's `externals` against what `public/` actually serves (#281).** Files under `staticDirs` are served at the Storybook root but sit outside every story's module graph, so TurboSnap cannot see a change to one — which is what `externals` is for. Candor does not set it, and the check is what makes that safe rather than merely current.

  **Setting it today would be wrong, not just unnecessary.** Chromatic captures the story iframe, and nothing in `public/` reaches it: the two favicons are consumed by the manager chrome and by `src/index.html`, and `preview-head.html` — the one file injected into the iframe — pulls Google Fonts and inline CSS and references neither. `externals` would therefore spend a full 292-snapshot rebuild on every favicon edit to guard a visual change that cannot occur. #281 had it listed as "low value"; measured against what the files are, it is negative value.

  **What flips it is a story rendering one of those assets**, and that is the reason for a gate rather than a note. TurboSnap would bypass a build in which the image genuinely changed, and the stale snapshot would pass with nothing reporting it — the #218 shape. The check fails if any story, MDX page or preview file references a `public/` asset by path while `externals` stays unset.

  The scan requires a **path context** — `src=`, `href=`, `url()`, or a leading slash — rather than a bare filename, because `editor-example.stories.ts` prints `hero-banner.png` as visible copy inside a mock editor. That is prose about a file, not a request for one: the same distinction the `12px-ok:` marker and the `aria-label` native-control scan both turn on. Verified in three directions rather than only the passing one — a story referencing an asset fails, the same story passes once `externals` is set, and the bare filename as visible copy does not fire.
