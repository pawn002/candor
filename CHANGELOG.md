...
- **Build (`@candor-design/web-components`):** `npm run build:wc` failed under Vite 7 ("Multiple entry points are not supported when output formats include umd"). The main library (ESM + UMD) and the `tone-data` entry (ESM-only) are now built in two passes (`vite.wc.config.ts` + `vite.tone-data.config.ts`). The `./tone-data` subpath was also added to the package `exports` map, so the documented `@candor-design/web-components/tone-data` import now resolves for consumers.
