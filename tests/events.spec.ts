 ...
 *   • the deprecated aliases still fire, with their original (live) semantics;
 ...
 */
async function tally(page: Page, hostSelector: string, legacyName?: string): Promise<void> {
  await page.evaluate(
    ({ hostSelector, legacyName }) => {
      const host = document.querySelector(hostSelector)!;
      const w = window as unknown as { __tally: Tally };
      w.__tally = { input: 0, change: 0, legacy: 0, lastInput: null, lastChange: null };
      ...
      if (legacyName) {
        host.addEventListener(legacyName, () => { w.__tally.legacy++; });
      }
    },
    { hostSelector, legacyName },
  );
...
test.describe('candor-input — input (live) vs change (commit)', () => {
  test('streams one input per keystroke; change fires once on blur', async ({ page }) => {
    ...
    await field.waitFor();
    await tally(page, 'candor-input', 'input-change');
    ...
    expect(t.legacy).toBe(3);        // deprecated `input-change` tracks the live event
...
test.describe('candor-slider — input floods, change fires once on release', () => {
  test('a pointer drag emits many input events but a single change', async ({ page }) => {
    ...
    await range.waitFor();
    await tally(page, 'candor-slider', 'value-change');
    ...
    expect(t.change).toBe(1);
    // The deprecated `value-change` tracks the live stream 1:1.
    expect(t.legacy).toBe(t.input);
