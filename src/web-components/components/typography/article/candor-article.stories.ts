...
import '../../code/candor-code';
import './candor-article';
...
## Signalling AI-generated content: the \`justify\` attribute

Add the \`justify\` attribute to apply full justification and automatic hyphenation to paragraph text:
...
**Why justify signals AI origin.** Full justification creates clean block edges — a typographic register associated with formal, produced documents (legal texts, official reports, published books). Human-authored notes and prose carry a natural ragged right edge. This visual distinction allows readers to immediately and consistently identify AI output without relying on labels alone.

This is a **systems transparency feature**, not a stylistic preference. In critical workflows — planning decisions, medical summaries, legal briefs — users must never have to read carefully to determine whether content was written by a human or generated by a model. The typographic register makes the distinction automatic and ambient.

The formal register does not make AI output feel cold. The content can still be written warmly. Typography signals *origin and authority*; tone is a content strategy decision.

**Requirements:**
- Add \`lang="en"\` (or the appropriate language tag) to the \`<candor-article>\` element or an ancestor — required for \`hyphens: auto\` to look up hyphenation dictionaries.
- Justification applies to \`<p>\` elements only — headings remain left-aligned.
- Use with \`font="serif"\` for authored prose contexts; combining with \`font="sans"\` is valid for formal UI documentation but uncommon.
        `.trim(),
      },
    },
  },
  argTypes: {
    font: {
      control: 'select',
      options: ['serif', 'sans'],
      description: 'Body typeface — Noto Serif (serif, default) or Noto Sans (sans, for UI paragraph contexts). Headings always use Roboto Flex.',
    },
    justify: {
      control: 'boolean',
      description: 'Full justification for AI-generated or formal document prose. Signals produced-artifact origin — use whenever the content source should be visually unambiguous. Requires lang attribute on the element or an ancestor for hyphenation.',
    },
  },
  args: { font: 'serif', justify: false },
  render: (args) => html`<candor-article font="${args['font']}">
...
export const SansSerif: Story = {
  render: () => html`<candor-article font="sans">
...
export const WithCode: Story = {
  render: () => html`<candor-article font="serif">${codeArticleContent}</candor-article>`,
};
...
export const AIGeneratedProse: Story = {
  render: () => html`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:var(--spacing-md);max-width:900px;">
      <div style="background:var(--color-bg-surface);border:var(--border-width-thin) solid var(--color-border-subtle);border-radius:var(--radius-md);overflow:hidden;">
        <div style="padding:var(--spacing-sm) var(--spacing-md);border-bottom:var(--border-width-thin) solid var(--color-border-subtle);display:flex;align-items:center;gap:var(--spacing-xs);">
          <span style="font-family:var(--font-family-base);font-size:var(--font-size-sm);font-weight:var(--font-weight-semibold);color:var(--color-text-default);">✍ Planning officer notes</span>
          <span style="font-family:var(--font-family-base);font-size:var(--font-size-sm);color:var(--color-text-subtle);margin-left:auto;">Human-authored</span>
        </div>
        <div style="padding:var(--spacing-md);">
          <candor-article font="serif">
            <p>The traffic issue kept coming up — B4632 is already at capacity at peak times and the applicant's figures didn't really account for the school run. That needs independent scrutiny before we can move forward.</p>
            <p>Heritage wasn't as contentious as I expected. The conservation officer seemed satisfied with the brick specification and I think the committee agreed. The green-space offset is the other outstanding thing — the numbers feel thin but it may just need better presentation.</p>
            <p>My recommendation is to defer pending the transport study. Eight weeks is tight but workable if the applicant commissions it promptly.</p>
          </candor-article>
        </div>
      </div>
      <div style="background:var(--color-bg-surface);border:var(--border-width-thin) solid var(--color-border-subtle);border-radius:var(--radius-md);overflow:hidden;">
