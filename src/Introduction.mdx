...
Regulatory exposure, outcome validation, and research comparison are different questions — no single algorithm answers all three. OKCA is the primary method: WCAG-compatible, coherent with OKLCH, and structured around a size-and-use-case tier system. See [Design Tokens / Typography → Contrast Guidance](?path=/story/design-tokens-typography--contrast-guidance) for the full tier table.
