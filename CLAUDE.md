...
**"Runs in CI" and "blocks a merge" are different claims, and right now only the first is true.** GitHub's required-status-check list on `main` contains `build-tokens (20)` and `build-tokens (22)` and nothing else. The `audit`, `typecheck`, `accessibility` and `chromatic` jobs all run and all report, but a PR can be merged with any of them red. So every gate listed above is enforced *to a reader of the checks page*, not by the branch — which is the same gap between claimed and actual guarding that this whole area keeps producing, one level up from the scripts. Do not describe a check as blocking until it is in that list.
