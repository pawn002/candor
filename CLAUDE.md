...
2. **`@candor-design/web-components`** — the primary, canonical consumer-facing component library: 37 Lit 3 components, registering 40 custom elements (`candor-tabs`, `candor-toast` and `candor-toolbar` each register a companion element). **Both counts are correct and they count different things** — say which noun you mean, since using them interchangeably is what put three different figures into circulation at 5.0.1 (#267). `npm run audit:docs` checks both. Framework-agnostic; the WC stories under `Components/`, `Typography/`, `Design Tokens/`, and `Examples/` are the only component surface.
...
**The gate.** `npm run audit:tokens` fails if any `oklch()` literal anywhere in `src/` is out of gamut, and prints the in-gamut value to use. It walks every `.ts` and `.scss` under `src/` — not just the token directory, and not just files the artifact is built from. **A colour does not have to be a token to be an authored Candor colour**: a gradient stop in a story, a heat-map fill in a demo, and a `--color-*` declaration are equally unrenderable, equally undefined for OKCA, and equally published under Candor's name. The gate scanned only `src/design-tokens/` until #228, when 16 of the 36 `oklch()` literals outside it turned out to be out of gamut — including a lightness ramp in the colour-tool example whose own gamut matrix, rendered a few lines below it, already recorded the endpoints as unrenderable. There is deliberately **no opt-out**: inspection found no legitimate out-of-gamut value, and an exemption with no user is only an escape hatch waiting to be used.

Two things the scan does not judge, stated so they are not assumed covered: template interpolation (`oklch(${l} ${c} ${h})`), which has no value until it runs; and relative colour syntax (`oklch(from var(--token) …)`), which derives from a token already gated here. The four sites that shift *lightness* under that syntax while holding chroma are the shape that can genuinely leave the gamut — all four measured in gamut in both modes at #228, because the tokens involved carry chroma 0 or 0.03, but that is a fact about the current values rather than a guarantee. Check a single colour with `klar contrast <c> "#000" --allow-out-of-gamut --json` → `.gamut.outOfGamut`. **Never use culori's `clampChroma`/`toGamut`** for this — they answer a different question (what to substitute) rather than the only one Candor asks (is this value renderable at all).
...
- Components register themselves on import via `@customElement('candor-{name}')` — pulling `src/web-components/index.ts` is enough to register all 40 tags.
...
Two machine-readable files in `audit/` serve as the canonical inputs for contrast audits. Keep them current as tokens and components evolve. `npm run audit:contrast` (`scripts/check-contrast.js`) is what reads them — it re-measures every pairing against its `min`, and every OKCA figure recorded in a token comment, in a pairing's `note`, or in story prose, each against the current token values, and prints `UNCHECKED` for anything it can't interpret rather than passing it silently. It exits non-zero on any enforced failure — `1` for a violation, `2` when the audit could not be trusted to run at all (klar missing, wrong major, or a colour outside sRGB).
...
**Both run in CI**, in that order, in the single `audit` job — so the sRGB invariant and every contrast floor are enforced on each PR rather than depending on someone remembering. The job re-exports the artifact and fails if it differs from the committed one, catching a token change that was never re-exported, and only then measures contrast.
...
**"Runs in CI" and "blocks a merge" are different claims — check which one you mean.** Until August 2026 only the first was true here: the required-status-check list on `main` held `build-tokens (20)` and `build-tokens (22)` and nothing else, so `audit` could go red and the PR would merge anyway. Every gate above was enforced *to a reader of the checks page*, not by the branch — the same gap between claimed and actual guarding this area keeps producing, one level up from the scripts.

It now holds **`build-tokens (20)`, `build-tokens (22)`, `audit`, `typecheck`, `accessibility`**, with `strict: true` so a branch must be current with `main` before it merges. `chromatic` is deliberately excluded: a visual diff needs a human to accept it, so requiring it would convert every intentional design change into a blocked merge.

Two limits on what that buys, because the list overstates itself otherwise. `enforce_admins` is `false`, so an admin can still merge past a red check — required checks make that deliberate rather than silent, which is the useful property, but it is not a wall. And a required check only constrains merges *through a PR*; a direct push to `main` is governed by different settings. **Do not describe a check as blocking until it is in that list**, and check the list rather than this paragraph — this is prose, and prose is what the rest of this section is about.
...
**`web-components/custom-elements.json`** — auto-generated, do not edit by hand, and **committed** (unlike `web-components/dist`, which is gitignored).
- Produced by `npm run build:cem`, configured in `custom-elements-manifest.config.mjs`. Regenerate after any change to a component's source or its TSDoc; CI runs `build:cem` and fails on a diff, the same gate `audit/tokens.dtcg.json` carries.
- Shipped in the tarball and declared via the `customElements` field, so VS Code and JetBrains pick it up. It adds ~25 kB to the package (118 → 143 kB) and never reaches a browser — it is a dev-time artifact.
- `cssParts` and `cssProperties` are **derived from the source**, not from `@csspart`/`@cssprop` tags: a `part="input"` in the template is already the declaration, and part names are public API under semver, so a hand-written tag that drifts is a false promise. Attribution is per class node, because three modules define two elements each and `candor-toast.ts` is the live case.
- Four config plugins, each load-bearing — read the file before removing one. Two exist purely to make the artifact reproducible: the analyzer emits modules in a **non-deterministic order** (verified: three runs, three orders), and it copies TSDoc text verbatim, so a Windows checkout produces CRLF a Linux CI run does not. Without either fix the staleness gate fails at random on changes that touched nothing.
...
- **The same command enforces the sRGB gamut invariant** and exits 1 with the in-gamut value to use. It scans every `.ts` and `.scss` under `src/` — far wider than it exports from, so `syntax.scss`, component `static styles`, and story gradients are all covered even though none of them is in the artifact (#228). `--skip-gamut` bypasses it (klar not installed); nothing in the repo passes that flag.

**`audit/pairings.json`** — hand-authored, update alongside component changes.
- 121 foreground/background pairings, each measured in both modes; 4 are `exempt`, leaving 238 enforced measurements. Per-component coverage is not derivable from the entry ids — form controls share `form-*` prefixes — so treat "every component is covered" as unverified until #197 settles how coverage is counted. **Read these counts as of a date, not as a fact about the file** — this sentence said "108 / 216" while the file held 121 / 238, having drifted during the 5.0.0 work that added the pairings. The numbers are computable from `pairings.json` in one line; until something computes them, prefer counting to quoting. #217 is the standing example of what the coverage gap costs: zero of the 117 pairings then on file referenced any border token, so the colour drawing the edge of eleven form components had no floor.
...
- **`note`** — free prose, with one exception: **an OKCA figure in a note is audited (#239).** Write `OKCA <n> light / <n> dark`, or `OKCA <n>` alone for light. No anchor is needed — unlike story prose, a note belongs to a pairing, so its fg and bg are already known. That is also the constraint: inside a note, `OKCA <n>` always means *this pairing's* colours, so a figure about any other colour must not be written in that grammar or the audit will confidently measure it against the wrong pair. `was OKCA <n> dark` marks a superseded figure and is skipped — notes need this more than token comments do, since a note is often the record of *why* a colour moved and quotes the number it used to have. A figure adjacent to a mode word but missing the `OKCA` keyword (`light 10.3, dark 6.1`) is reported as `UNCHECKED`. Nothing read these fields until #239, and **16 of 28 figures had drifted** — mostly through the klar 2→3 re-baseline that was applied everywhere the audit could see, which is the tell.
...
**This is gated by `tests/host-aria-label.spec.ts` in the `accessibility` job (#270), and it has to be a runtime test.** The failure it guards is *absence*: an unnamed control renders pixel-identically to a named one, so Chromatic cannot see it, and the contrast and gamut audits do not look at naming. Six of the ten form controls forwarded and four did not — for a year — while `candor-input`'s own story instructed consumers to use `aria-label` on the host. Nothing contradicted it, because nothing was reading.

The spec asserts, for each subject and in **both** insertion orders (attribute-then-append, the plain-HTML path; and append-then-attribute, the React/Angular path the MutationObserver exists for): the host no longer carries the attribute, exactly one inner element carries it, that element's *computed accessible name* is the value, and — where a native control exists — the native control is the element that got named.

**Its subject list is derived from source, and needs both halves of the derivation.** A component qualifies if it renders a native `<input>`/`<textarea>`/`<select>` (the **obligation** — this is what makes the rule apply, so a component cannot leave the set by simply not forwarding, and it is the half that catches the #270 bug) **or** if it calls `observeHostAriaLabel` (the **coverage** — it pulls in components whose role sits on a `<button>` or `<ul>`, like `candor-listbox` and `candor-menu`). The second half alone is circular: it can only check components that already claim to forward.
...
A deliberate non-fix: there is **no dev-mode `console.warn` for an unnamed control**, though it is the obvious third remedy. React and Angular attach an element and then set its attributes, so the warning would fire on every correctly-named control in those frameworks and train people to ignore the console — which costs more than it catches.
...
Tokens re-theme the whole system but can't reach *one* component's internals. Two opt-in hooks let consumers override without forking (#165). Both must leave default rendering identical — set nothing, nothing changes.

**This convention governs new components; it is not a property every component currently has.** Measured across the 37: 5 expose a `::part`, 9 expose a `--candor-*` property, 4 expose both, and **27 expose neither** (#246). The sentence here used to read "every component exposes two opt-in hooks", which described 4 of 37 — and mattered more than a wording slip, because the governance paragraph below declares these names public API with semver obligations, so a contributor reasonably read it as a surface that already existed and was maintained. `src/Introduction.mdx` states this correctly and is the model: it lists what exists and says the surface grows as demand surfaces. **Growing it by demand is the design, not a backlog.** Every hook that exists came from real friction — `--candor-drawer-size` from a consumer needing a wider drawer, `::part(trigger)` from #173, `--candor-data-grid-cell-min-height` from #229 — and a part name is public API, so inventing 27 components' worth of them without a use case is 27 commitments made blind. That is the #213 `redundant-channel` mistake: populating a surface because a convention says it should be populated produces something that looks designed and isn't.
...
16. **Don't write a CSS comment inside a `style="…"` attribute** — and if you must, never let it contain a double quote. A `"` inside the comment closes the attribute early, so every declaration *after* the comment is silently discarded. Nothing errors at any layer: lit-html interpolates the string, the browser parses the attribute it was handed, and the story renders. This is not hypothetical — the colour showcase lost `overflow-wrap: break-word` and `margin: 0` this way, and the visual check passed because removing `word-break: break-all` was the load-bearing half of the same fix, so the symptom the comment described did look repaired.

    **Put the prose in TypeScript instead**, where quoting is free: build the declarations as a `const` array joined into the attribute (`SWATCH_LABEL_STYLE` in `color-showcase.stories.ts` is the pattern), and comment it as ordinary code. The repo currently holds four in-attribute comments, all `12px-ok:` markers that must sit in a comment to satisfy the text-size gate, and none contains a quote — verify that stays true if you add one. Note the general shape: **a failure whose only symptom is absence is invisible to a screenshot.** Read the computed style when a CSS fix is meant to have changed layout, rather than trusting that the picture looks right.
...
- `tests/host-aria-label.spec.ts` — every component that must forward a host `aria-label` actually does, in both insertion orders (#270). **Its subjects are derived from source, not listed** — see "aria-label host-trap" above for the two-half derivation and why a hand-written list would be a false negative waiting to happen. It builds its own probe elements with `document.createElement` rather than relying on a story's props, so it needs no per-component story and adds no Chromatic snapshot. A `discovery` test asserts the subject count has a floor, so a broken scan fails loudly instead of passing by testing nothing.
